...
@Component
public final class RetryLimitRuntimeGuardrailPolicy
        implements RuntimeGuardrailPolicy {

    static final String POLICY_NAME = "retryLimit";
    ...
    public RuntimeGuardrailResult evaluate(
        ...
        if (operation != RuntimeGuardrailOperation.RETRY) {

            return RuntimeGuardrailResult.allow(
                    POLICY_NAME);
        }

        if (budget.retriesUsed() >= budget.limits().maxRetries()) {
